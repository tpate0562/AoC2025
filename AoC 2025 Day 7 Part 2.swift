...
guard let fileContent = readInputFile(at: inputFilePath) else {fatalError()}
...
var cols = 141, rows = 141, counts: [Int] = Array(repeating: 0, count: cols)
...
for i in 1..<cols {
    for j in 0..<rows {
        if statuses[j] == true && array[i][j] == "^"{
            statuses[j] = false; statuses[j-1] = true; statuses[j+1] = true; counts[j-1] += counts[j]; counts[j+1]+=counts[j]; counts[j]=0; print(i,j)
        }
    }
}
